...
class QLSTM(keras.layers.Layer):
    def __init__(
        ...
        self.feature_combiners = [keras.layers.Dense(self.wires, dtype=dtype_global) for _ in range(4)]
        self.hidden_dense = keras.layers.Dense(self.units, dtype=dtype_global)
    ...
    def call(self, inputs: tf.Tensor, *args, **kwargs):
        ...
        self.h = self.hidden_dense(tf.zeros((batch_size, self.wires), dtype=dtype_global))
        ...
        # B*t*W -> (B, W, t) ---Transpose--> (B, t, W)
        y_out = tf.reshape(y_out, ( batch_size, self.units, inputs.shape[1]))
    ...
    def __get_circuit(self) -> qml.QNode:
        ...
        return qml.QNode(circuit, self.dev, diff_method='adjoint', interface='tf')
    ...
    def lstm(self, inputs: tf.Tensor, c_t_1) -> tf.Tensor:
        ...
        h_t = self.hidden_dense(o_t*tf.tanh(c_t))
